 ...
 * @since 2020/3/30
 */
@Rule(name = "mqttUnSubscribeRule", description = "mqtt unSubscribe 报文匹配规则")
public class MqttUnSubscribeRule extends AbstractRule {
    ...
    public boolean condition(@Fact(DEFAULT_FACTS_NAME) RulePacket packet) {
        return Optional.ofNullable(packet.getSrcBuffer()).map(buffer -> {
            ...
            return packetType == MqttPacketTypeEnum.UNSUBSCRIBE.packetType;
        }).orElse(Boolean.FALSE);
    }

    @Action
    public void action(@Fact(DEFAULT_FACTS_NAME) RulePacket packet) {
        packet.setData(mqttParser.decodeMqttUnSubscribePacket(packet.getSrcBuffer(), packet.getChannel()));
